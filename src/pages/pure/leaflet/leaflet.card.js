...
L.Control.Card = L.Control.extend({
  ...
  initialize: function (options) {
    ...
    this._filled = 0
    this._max = Math.floor((window.innerHeight - 200) / 110)
  },
  onAdd: function (map) {
    const { isFace } = this.options
    this._map = map
  ...
  insertImage: function (fromTopLeft, imageUrl) {
    ...
    this._initialWidth = isFace ? 12 : 20
    img.src = imageUrl
    img.style.cssText = `
        border: 1px solid red;
        position: fixed;
  ...
  _move: function (img, fromTopLeft, destinyTopLeft, destiny) {
    ...
    const destinyWidth = isFace ? 98 : 170
    const xyChange = 2
